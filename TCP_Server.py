...
bind_ip = "0.0.0.0"     # 代表本网络所有主机
bind_port = 9999        # 监听9999端口
...
# 让服务端进入主循环，在这里等待连接，当与一个客户端建立连接的时候，将接收到的客户端套接字对象保存到client变量中，将远程连接的细节保存到addr变量中，然后以handle_client函数为回调函数创建一个新的线程对象
while True:
